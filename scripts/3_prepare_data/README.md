...
Switch to Levenshtein-based synonym selection:
```bash
...
| `--selection-method` | `embedding` | How to pick the best annotation: `levenshtein` or `embedding`. |
| `--encoder-name` | `GanjinZero/coder-all` | Sentence Transformer model for embedding-based selection. |
...
## Outputs
For each processed dataset (e.g., `MedMentions`), a folder is created under `--out-root` containing separate pickle files for each data split:
```
<out_root>/
├── MedMentions/
│   ├── train_source.pkl
│   ├── train_target.pkl
│   ├── validation_source.pkl
│   ├── validation_target.pkl
│   ├── test_source.pkl
│   ├── test_target.pkl
